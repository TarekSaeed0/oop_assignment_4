...
public class FilterByBodyCriterion extends MailCriterion {
	private final String searchBy;

	public FilterByBodyCriterion(String searchBy) {
	...
	public List<Mail> meetsCriterion(List<Mail> mailList) {
		...
		for(Mail mail: mailList) {
			if(mail.getData().getBody().toLowerCase().contains(searchBy)) {
				filtered.add(mail);
			}
		}
		return filtered;
